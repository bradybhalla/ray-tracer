...
let get_diffuse_color scene int l tex =
  let color = Texture.eval tex int.si.tex_coord in
  let sample = Light.sample_Li l int.si in
...
let whitted (scene : Scene.t) (initial_ray : Ray.t) =
  let rec helper (ray : Ray.t) (max_depth : int) =
    if max_depth = 0 then Vec3.zero ()
      ...
      | None -> sum_over (fun il -> Light.get_Le il ray) scene.infinite_lights
      | Some int -> (
